...
def parse_vtt(content: str) -> List[SubtitleCue]:
    ...
    for i, line in enumerate(lines):
        ...
        if match:
            ...
            # Simple heuristic for ID: if pre-line is a number or short string
...
def generate_vtt(cues: List[SubtitleCue]) -> str:
    ...
    for cue in cues:
        ...
        # output.append(f"{cue.id}") # ID is operational in VTT, some players like it, some don't. Keep it for safety if reliable.
        # Check if ID looks like a real ID (number) or garbage. 
        # Let's print ID if it exists.
        if cue.id: output.append(f"{cue.id}")
...
def translate_batch(
    ...
    try:
        response = client.models.generate_content(
            model='gemini-2.5-flash', # Or gemini-1.5-flash if 2.5 not avail
            contents=prompt,
            config={
